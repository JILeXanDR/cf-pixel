...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (reqCookies.token) {
        try {
          const tok = base64ToBytes(stripQuotes(reqCookies.token));
          if (tok.length >= 16) {
            ...
            if (json && typeof json.id === "string" && json.id) visitorId = json.id;
          }
        } catch (e) {
      ...
      if (!visitorId) {
        // Mint new cookie
        visitorId = genXid();
        const payload = new TextEncoder().encode(JSON.stringify({ id: visitorId, ts: new Date().toISOString() }));
        const iv = crypto.getRandomValues(new Uint8Array(16));
        const cipher = await aesCfbEncrypt(keyBytes, iv, payload);
        const b64 = bytesToBase64(concat(iv, cipher));

        // use cookie.serialize for correctness
        headers.append(
          "Set-Cookie",
          serializeCookie("token", b64, {
            path: "/",
            maxAge: 31536000, // 1y
            httpOnly: true,
            secure: true,
        ...
        // If you want zero caching on first set-cookie, uncomment:
        // headers.set("Cache-Control", "no-store");
      }

      const body = `window._PH_VISITOR_ID='${escapeJs(visitorId)}';\n`;
...
async function aesCfbEncrypt(keyBytes, iv, plaintext) {
  ...
  for (let o = 0; o < plaintext.length;) {
    const ks = new Uint8Array(await crypto.subtle.encrypt({ name: "AES-CBC", iv: ivBlock }, key, zero));
